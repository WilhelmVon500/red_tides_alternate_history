...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['nlp', 'kpd', 'uspd', 'spd', 'fvp', 'bb', 'z', 'wp', 'rlb', 'dnvp', 'dap', 'other'];
    }
    if (!partyColors) {
        partyColors = {'kpd': '#8B0000', 'uspd': '#D47B9B', 'spd': '#E3000F', 'fvp': Q.fvp_color, 'bb': '#38C538', 'z': '#000000', 'nlp': Q.nlp_color, 'csvd': '#1DB4E1', 'wp': '#B8996F', 'rlb': '#52AE6F', 'other': '#909090', 'kvp': '#455B95', 'dnvp': '#3F7BC1', 'kswr': '#003755', 'dap': '#096300'};
    }
    if (!partyNames) {
        partyNames = {'kpd': 'KPD', 'uspd': 'USPD', 'spd': 'SPD', 'fvp': Q.fvp_name, 'bb': 'BB', 'z': 'Z + BVP', 'nlp': Q.nlp_name, 'csvd': 'CSVD', 'wp': 'WP', 'rlb': 'RLB', 'other': 'Others', 'kvp': 'KVP', 'dnvp': 'DNVP', 'kswr': 'KSWR', 'dap': 'DAP'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
